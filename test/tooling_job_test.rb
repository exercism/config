...
module Exercism
  class ToolingJobTest < Minitest::Test
    ...
    def test_queues_job_in_background
      ...
      job = ToolingJob.create!(id, type, submission_uuid, :ruby, "two-fer", run_in_background: true)

      assert redis.get("submission:#{submission_uuid}:#{type}")
      assert redis.get("job:#{id}")
      assert_equal id, redis.lpop(ToolingJob.key_for_queued_for_background_processing)
    end
